...
*   Operators (2026-08-03): `+ - * /`, `== != < <= > >=`, `&& || and or` (real short-circuit), `! not`, unary `+ -`. Every number is coerced to `float64`, including in comparisons. Mixed-kind comparisons and COM-object operands are errors, never a silent false. Deliberately absent: `in`, `matches`, `contains`, `startsWith`, `endsWith`, `..`, `??`, `?:`.
...
* ~~The `expression` package supported only `+ - * /`: `a > b` parsed fine and then failed at runtime with "unsupported binary operation".~~ — Added `== != < <= > >=` (in `evalBinary`, via `compare`) and `&& || and or` (in `eval`, via `evalLogical`). Numbers compare through the existing `toFloat` coercion, so every comparison is a float64 comparison (`0.1+0.2 == 0.3` is false, and a uint64 above 2^53 loses precision — documented rather than special-cased, to keep one numeric model); strings compare lexicographically; bools support equality only; nil supports equality only, so `x == nil` is the empty-cell test. A mixed-kind comparison keeps the existing unsupported-operation error naming both types — a permissive fallback (comparing `fmt.Sprint` of both sides) would make every one of them "work" and silently answer wrong. Explicitly OUT and now documented as a boundary: `in`, `matches`, `contains`, `startsWith`, `endsWith`, `..`, `??`, and `?:` (a distinct `ast.ConditionalNode` needing its own arm in `eval`).
  ...
  * **A COM object operand is refused, and the check is load-bearing rather than cosmetic.** `Chain.Value()` answers `(nil, nil)` for a chain holding a dispatch but no VARIANT result (any root from `Create`/`From`/`Fork`), so `someObject == nil` would have evaluated to **TRUE for a live COM object**. The shared `unwrapChainValue` (extracted from the three hand-rolled copies in `evalUnary` / `evalBinary` / the new short-circuit path) tests `IsDispatch()` first and errors. Object identity comparison stays out of scope.
